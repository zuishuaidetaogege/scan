...
const generateRouterMap = [
  {
    path: '/scan',
    redirect: '/scan/palletinfo/list',
    component: RouteView,
    name: 'scan',
    meta: { title: '扫描', icon: 'barcode', permission: ['scan'] },
    children: [

      {
        path: '/scan/palletinfo/list',
        name: 'palletinfoList',
        component: () => import('@/views/scan/palletinfo/list'),
        meta: { title: '包装详细', keepAlive: true, permission: ['Palletinfo'] }
      }
